...
SVMModel = fitcsvm(normalized_train_data',train_label,'KernelFunction','gaussian','KernelScale',27.2223);
%model_1 = fitcsvm(normalized_train_data',train_label,'KernelFunction','gaussian') 
...
hold on
scatter(g_2(:,1),g_2(:,2),'ro','filled')
title 'Gaussian kernel SVM classifier labels'
legend('\bf{1}','\bf{0}')
...
test_n_of_0_misclassification = sum(t_labels(t_labels==0)~=test_label(t_labels==0)) ;
test_n_of_1_missclassification= sum(t_labels(t_labels==1)~=test_label(t_labels==1));
