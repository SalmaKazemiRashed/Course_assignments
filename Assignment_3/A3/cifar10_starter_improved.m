function cifar10_starter_improved()
    addpath(genpath('./'));
    ...
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,3,20)/sqrt(5*5*3/2), 'biases', zeros(20,1)),...
        'padding', [2 2]);
   ...
   net.layers{end+1} = struct('type', 'maxpooling');
    ...
    %% new network
       net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,20,40)/sqrt(5*5*20/2), 'biases', zeros(40,1)),...
        'padding', [2 2]);
       net.layers{end+1} = struct('type', 'relu');
       net.layers{end+1} = struct('type', 'maxpooling');
       %%
       ...
       net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,40,80)/sqrt(5*5*40/2), 'biases', zeros(80,1)),...
        'padding', [2 2]);
     net.layers{end+1} = struct('type', 'relu');
       net.layers{end+1} = struct('type', 'maxpooling');
       %%%
       ...
       net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,80,16)/sqrt(5*5*80/2), 'biases', zeros(16,1)),...
        'padding', [2 2]);
     net.layers{end+1} = struct('type', 'relu');
       %net.layers{end+1} = struct('type', 'maxpooling');
    
      net.layers{end+1} = struct('type', 'fully_connected',...
        'params', struct('weights', randn(10,256)/sqrt(256/2), 'biases', zeros(10,1)));
...
%   
    ...
    save('models/cifar10_baseline_less_lr.mat', 'net');
    ...
    for i=1:batch:size(y_test)
     ...
     save('models/cifar10_result_less_lr.mat', 'pred');
